...
# Test imports
from csp_adapter_slack.v1 import (
    SlackAdapter,
    SlackAdapterConfig,
    SlackBackend,
    SlackConfig,
...
class TestSlackAdapter:
    ...
    def test_adapter_creation(self, mock_config):
        ...
        with patch("csp_adapter_slack.v1.adapter.SlackBackend") as mock_backend_class:
...
@pytest.mark.skipif(not HAS_CSP, reason="CSP not installed")
class TestSlackAdapterCSPGraphs:
    """Tests for SlackAdapter CSP graph integration."""
    ...
    def test_subscribe_graph_creation(self, mock_config):
        """Test that subscribe creates a CSP graph."""

        with patch("csp_adapter_slack.v1.adapter.SlackBackend") as mock_backend_class:
            mock_backend = MagicMock()
            mock_backend.config = mock_config
            mock_backend.name = "slack"
            mock_backend_class.return_value = mock_backend

            adapter = SlackAdapter(mock_config)

            # The subscribe method should return a time series node
            # We'll test this by checking the returned type in a graph
            @csp.graph
            def test_graph():
                # Subscribe returns a ts[[Message]] - we just need to create the graph
                messages = adapter.subscribe()
                # Add an output so the graph is valid
                csp.add_graph_output("messages", messages)

            # Just verify the graph can be constructed (don't run it)
            # The graph construction exercises the subscribe method
            assert test_graph is not None

    def test_subscribe_with_channels(self, mock_config):
        """Test subscribe with channel filter."""
        with patch("csp_adapter_slack.v1.adapter.SlackBackend") as mock_backend_class:
            mock_backend = MagicMock()
            mock_backend.config = mock_config
            mock_backend.name = "slack"
    ...
    def test_publish_graph_creation(self, mock_config):
        """Test that publish creates a CSP graph."""
        with patch("csp_adapter_slack.v1.adapter.SlackBackend") as mock_backend_class:
            mock_backend = MagicMock()
            mock_backend.config = mock_config
            mock_backend.name = "slack"
    ...
    def test_publish_reaction_graph_creation(self, mock_config):
        """Test that publish_reaction creates a CSP graph."""
        with patch("csp_adapter_slack.v1.adapter.SlackBackend") as mock_backend_class:
            mock_backend = MagicMock()
            mock_backend.config = mock_config
            mock_backend.name = "slack"
            ...
            def test_graph():
                ...
                emoji = csp.const("thumbsup")
                adapter.publish_reaction(msg, emoji, timeout=1.0)

            assert test_graph is not None

    def test_publish_presence_graph_creation(self, mock_config):
        """Test that publish_presence creates a CSP graph."""
        with patch("csp_adapter_slack.v1.adapter.SlackBackend") as mock_backend_class:
            mock_backend = MagicMock()
            mock_backend.config = mock_config
            mock_backend.name = "slack"
...
@pytest.mark.skipif(not HAS_CSP, reason="CSP not installed")
class TestSlackAdapterPresence:
    """Tests for SlackAdapter presence functionality."""
    ...
    def test_presence_status_values(self):
        """Test SlackPresenceStatus enum values."""
        # Test all presence statuses
        assert SlackPresenceStatus.ACTIVE.value == "active"
        assert SlackPresenceStatus.AWAY.value == "away"
        assert SlackPresenceStatus.AUTO.value == "auto"

    def test_presence_node_runs_in_graph(self, mock_config):
        """Test that presence node executes within a CSP graph."""
        with patch("csp_adapter_slack.v1.adapter.SlackBackend") as mock_backend_class:
            mock_backend = MagicMock()
            mock_backend.config = mock_config
            mock_backend.name = "slack"
            ...
            @csp.graph
            def test_graph():
                presence = csp.const(SlackPresenceStatus.AWAY)
...
class TestSlackAdapterReactions:
    """Tests for SlackAdapter reaction functionality."""
    ...
    def test_reaction_node_runs_in_graph(self, mock_config):
        """Test that reaction node executes within a CSP graph."""
        with patch("csp_adapter_slack.v1.adapter.SlackBackend") as mock_backend_class:
            mock_backend = MagicMock()
            mock_backend.config = mock_config
            mock_backend.name = "slack"
            ...
            def test_graph():
                ...
                emoji = csp.const("wave")
                adapter.publish_reaction(msg, emoji, timeout=0.1)
